...
async def observe(query: str, history: List[dict], memory_hits: List[MemoryItem]) -> Observation:
    ...
    5. Artifacts: If analyzing large data from a past step, set `attach_artifact_id` (e.g., 'art:123').
    6. Self-Check: Ensure goals are sequential, non-redundant, and directly answer the query.
    7. Fallbacks: If unsure or a tool failed, output a fallback goal to ask for clarification.
    8. Strict Output: Output ONLY the expected JSON schema.
    """
